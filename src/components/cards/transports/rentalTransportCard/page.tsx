    ...
    <div className="w-full">
      <Slider {...settings} className="rantal-arrows">
        {featuredCards?.map((transport) => {
          ...
          return (
          <Link href={`/cars/${transport?.slug}?${queryString}`} key={transport?.id} className="px-2">
            <Card className="w-full overflow-hidden bg-white rounded-[10px] shadow-none border">
              ...
              <div className="relative w-full h-36 md:h-[280px] sm:h-48">
                <Image
                  src={transport?.image_url || "/fallback-image.jpg"}
              ...
              <div className="md:pt-6 pt-1 md:px-5 px-3 text-dark">
                ...
                <CardHeader>
                  <h3 className="slick-item-title font-inter font-semibold md:text-lg text-[14px] md:leading-7 leading-3 text-dark">
                    {transport?.title || "Unknown Transport"}
                  </h3>

                  {/* Specifications */}
                  <div className="flex flex-wrap md:gap-4 gap-2 md:mt-4 mt-2">
                    {
                ...
                <div className="flex md:flex-row flex-col items-center justify-between h-[70px]">
                  ...
                  <div className="flex gap-2 items-center justify-between md:justify-end w-full">
                    <div className="text-dark">
                      <h4 className="font-semibold text-base">{transport?.review_data?.score_text}</h4>
                      <p className="text-sm">{transport?.review_data?.total_review} reviews</p>
                    </div>
                    <p className="bg-info rounded-sm px-2 py-2 text-white text-sm">
                      {String(transport?.review_data?.score_total)}
                    </p>
                  </div>
                </div>
              </div>
