...
const CarDriverCard = ({ data, basePath }: any) => {
  ...
  // Conditional check for rendering slider based on data availability
  ...
  const settings = {
    ...
    autoplaySpeed: 3000,
    centerMode: false,
    arrows: hasData, // Enable arrows only when there is data
  ...
  return (
    <div className="w-full">
      {/* Only render the Slider if there is data */}
      {hasData ? (
        <Slider {...settings} className="cardrive-arrows inner-driver">
          {data.filter((chauffeur: Chauffeur) => !!chauffeur?.image_url).map((chauffeur: Chauffeur, index: number) => (
            <div key={index} className="px-2">
              <Card className="bg-white rounded-[10px] shadow-none border">
                <div
                  className="relative w-full h-36 md:h-[280px] sm:h-48 cursor-pointer"
                ...
                <CardContent className="md:px-5 px-2 pb-3">
                  <div className="md:space-y-2.5 space-y-1.5">
                    ...
                    <Link href={`/${basePath}/${chauffeur?.slug}?${queryString}`}>
