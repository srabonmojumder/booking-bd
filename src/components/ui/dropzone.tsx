  ...
  const dropzone = useDropzone({
    ...
    onDrop(acceptedFiles, fileRejections, event) {
      if (props.onDrop) props.onDrop(acceptedFiles, fileRejections, event)
        ...
        const _files = [ ...(mautiple ? filesUploaded : []), ...acceptedFiles ]
        setFilesUploaded(_files)
        onChangeValue?.(_files)
  ...
  return (
    ...
    >
      <div
        ...
        className={cn('flex justify-center items-center w-full h-28 border-dashed border-2 border-gray-200 rounded-lg hover:bg-accent hover:text-accent-foreground transition-all select-none cursor-pointer', dropZoneClassName)}
      >
        <input { ...dropzone.getInputProps() } />
        {
          children ? (
            ...
            <div className='text-sm font-medium'>Upload files / Drop your files here!</div>
          ) : (
