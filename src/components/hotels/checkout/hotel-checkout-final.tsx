...
export default function HotelCheckoutFinal({
  bookingData,
}: {
  ...
  //   try {
  //     setIsLoading(true);
  //     const { data, error } = await confirmBooking(formData);
  //     if (error) {
  //       toast.error(`Error: ${error}`);
  //     } else if (data) {
  //       if (formData.payment_gateway === "offline_payment") {
  //         toast.success("Booking confirmed! You will pay at the property");
  //         router.push(`/booking/${bookingData.booking.code}/success`);
  //       } else if (formData.payment_gateway == "stripe") {
